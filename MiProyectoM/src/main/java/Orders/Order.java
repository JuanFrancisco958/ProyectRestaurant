...
public class Order implements Serializable{
		...
		double result=0;
		 if (Products!=null) {
			for (Integer item : Products) {
				...
				if (!c.searchProduct(item).getBundle().isEmpty()) {
					for (Integer i : Products) {
						for (Integer o : c.searchProduct(i).getBundle()) {
							if (c.searchProduct(o).getId()==o) {
								Syso.print("descuento aplicado");
								result-=0.5;
							}
						}
	...
	public String toString() {
		return "Order [client=" + client.getDni() + ", id=" + id + ", Products=" + Products + ", total=" + total
