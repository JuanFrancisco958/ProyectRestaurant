...
public class MainMenu implements IMainMenuController{
	...
	public static void menuClientes() {
		...
		case 2:
			String p=GUI.getDni();
			if (p.length()==9) {
				c.searchClient(p).addAddress(GUI.getAddress());
			...
			break;
		case 3:
			String p1=GUI.getDni();
			if (p1.length()==9) {
				c.searchClient(p1).setAge(GUI.getAge());
				RepositoryUtil.exportarC(c);
			}
	...
	public void newClient() {
		...
		try {
			String p=GUI.getDni();
			if (p.length()==9) {
				c.addClient(new Client(p, GUI.getString("Introduce el nombre:"), GUI.getAge(), GUI.getAddress()));
				RepositoryUtil.exportarC(c);
			}
	...
	public  double cashNoPayed() {
		...
		Syso.print("La cantidad es: "+result);
		return result;
	}
	@Override
	...
	public void deleteOrder(Client c) {
		...
		RepositoryUtil.importarO(o);
		try {
			List<Order> item=o.getOrdersByClient(c.getDni());
			Syso.printO(item);
			if (c!=null) {
				Iterator<Order> i=o.getAllOrders().iterator(); 
				int n=GUI.getint("Introduce el id");
				while (i.hasNext()) {
					if (n==i.next().getId()) {
						i.remove();
						Syso.print("Ha sido borrado correctamente");
